...
if(isset($_GET['delete_id']))
{
	...
	$stmt_select = $DB_con->prepare('SELECT img_name FROM mis WHERE item_id =:item_id');
	$stmt_select->execute(array(':item_id'=>$_GET['delete_id']));
	$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
	unlink("../item_images/".$imgRow['img_name']);
	

	$stmt_delete = $DB_con->prepare('DELETE FROM mis WHERE img_name =:img_name');
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			?>
                <tr>
                  <td class="zoom">
				<center> <img src="../item_images/<?php echo $img_name; ?>" class="img img-rounded"  width="65" height="50" /></center>
				 </td>
             		 	 <td><b>
								   <?php include('../Dashboard_Principal/connect.php');
			// $get_id=$_GET['id'];
          //  $cats=$row['Prepared_By'];
            $query7 = mysql_query("select * from miscategory where category_id='$category_id'") or die(mysql_error());
...
?>			</td>
				 ...
				 <a class="btn btn-info" href="editdata.php?edit_id=<?php echo $row['category_id']; ?>" title="Click for Edit" onclick="return confirm('Are you sure edit this Module?')"><span class='glyphicon glyphicon-pencil'></span> Edit&nbsp;Information</a>



				 
								 <a class="btn btn-primary" href="editdata2.php?edit_id=<?php echo $row['item_id']; ?>" title="Click for Edit" onclick="return confirm('Are you sure edit this Selected Module?')"><span class='glyphicon glyphicon-check'></span> Update&nbsp;Image</a>
                  <a class="btn btn-danger" href="?delete_id=<?php echo $row['img_name']; ?>" title="Click for Delete" onclick="return confirm('Are you sure to remove this Module?')"><span class='glyphicon glyphicon-trash'></span> Delete</a>
