...
{
	$id = $_GET['edit_id'];
	$stmt_edit = $DB_con->prepare('SELECT * FROM miscategory WHERE category_id=:category_id');
...
if(isset($_POST['btn_save_updates']))
{
	...
	if(!isset($errMSG))
	{
		$stmt = $DB_con->prepare('UPDATE miscategory
									 SET classname=:classname,
										dep_sec=:dep_sec
								   WHERE category_id=:category_id');
		$stmt->bindParam(':classname',$classname);
				$stmt->bindParam(':dep_sec',$dep_sec);
...
?>
    ...
    <tr>
    	<td><label class="control-label">Department Sections</label></td>
	 		<td>
	   <div class="form-group">
 
    <div class="col-sm-12">
       <select  class="form-control" id="inputPassword3" name="dep_sec" required>

	   	 	
	   
	   
        <option value="<?php echo $sup_id;?>"><?php echo $dep_sec;?>
        ...
        <?php
		include('connect.php');
        $query=mysql_query("select * from misdepfinal") or die (mysql_error());
