...
<div class="content-wrapper">
 ...
 ?>
	...
	$query = mysql_query("Select sa.id,sa.index_number,sa.full_name,sa.i_name,sa.gender,sa.section,sa.address,sa.phone,sa.email,sa.image_name,sa.b_date,sa._status,sa.reg_year,sa.reg_month,sa.reg_date,sg.grade_id FROM student sa LEFT JOIN student_grade sg ON sa.index_number=sg.index_number where sg.grade_id='$grade_id' and sa.section='$section'") or die(mysql_error());
			 while ($row = mysql_fetch_array($query)) {
				$countstudent = mysql_num_rows($query);
			 }
 ?>
...
$query = mysql_query("SELECT * FROM message where grade_id='$grade_id' AND section='$section' ORDER BY id DESC") or die(mysql_error());


while($row = mysql_fetch_array($query))
  {
 echo '<p style="word-wrap: break-word;">'.'<img src="../../'.$row['image_user'].'" class="zoom" width="70" height="70"<img/>'.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.'<span style="font-weight: bold;
	color: #3B5998;">'.$row['sender'].'</span><b>'. '&nbsp;&nbsp;' . $row['message'].'</b></p><b style="color:black; font-size:10px; text-align:right:90px;">'. $row['timesend'].'</b><br><b style="font-size:14px;">' . $row['department'].' ' . $row['section'].'</b>';
  }
