...
$query=mysql_query("SELECT item_id, policy_title, item_image, revisionno, img_name, department FROM mis ORDER BY department LIMIT $start, $limit");
...
<textarea style='text-align:center;background-color: white;' class='form-control' rows='1' disabled>".$query2['policy_title']."</textarea>
		...
		$stmt=$dbcon->prepare('SELECT item_id, item_image, policy_title, revisionno, img_name, department FROM mis WHERE category_id=:cid ORDER BY department');
