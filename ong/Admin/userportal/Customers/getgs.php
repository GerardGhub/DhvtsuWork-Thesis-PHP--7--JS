...
$query=mysql_query("SELECT item_id, policy_title, item_image, revisionno, img_name, department FROM gsdepartment ORDER BY department LIMIT $start, $limit");
...
<textarea style='text-align:center;background-color: white;' class='form-control' rows='1' disabled>".$query2['policy_title']."</textarea>
		...
		$stmt=$dbcon->prepare('SELECT item_id, item_image, policy_title, revisionno, img_name, department FROM gsdepartment WHERE category_id=:cid ORDER BY department');
...
<textarea style='text-align:center;background-color: white;' class='form-control' rows='1' disabled>".$policy_title."</textarea>
            ...
            <a class='fancybox-buttons' href='/adminportal/item_images/$img_name' data-fancybox-group='button' title='Page ".$id."- ".$policy_title."'>
					...
					<center><h4>  ".$department." </h4></center>
										...
										<a class='btn btn-block btn-danger' title='Download Selected Policy?' href='#'></span>&nbsp;RevisionNo&nbsp;".$revisionno."</a>
