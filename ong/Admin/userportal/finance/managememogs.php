...
<?php
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT item_image FROM memorandum WHERE item_id =:item_id');
		$stmt_select->execute(array(':item_id'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("item_images/".$imgRow['item_image']);
		
	
		$stmt_delete = $DB_con->prepare('DELETE FROM memorandum WHERE item_image =:item_image');
...
<?php include('../includes/financeleftbar.php');?>  
                    ...
                    <div class="main-page">
                            ...
                            <div class="row page-title-div">
                                <div class="col-md-6">
                                    <h4 class="fa fa-suitcase" style="font-size:22px;">Manage O&G Memorandum</h4>
	...
	if($stmt->rowCount() > 0)
	{
		while($row=$stmt->fetch(PDO::FETCH_ASSOC))
		{
			...
			<td><?php echo $department; ?></td>
				 ...
				 <a class="btn btn-info" href="editmemopo.php?edit_id=<?php echo $row['item_id']; ?>" title="Click for Edit" onclick="return confirm('Are you sure edit this Policy?')"><span class='glyphicon glyphicon-pencil'></span> Edit</a> 
