...
//	include('formatMoney.php');
	...
	{
		$id = $_GET['JRFseries'];
		$stmt_edit = $DB_con->prepare('SELECT * FROM jobrequestfinalpending WHERE JRFseries=:JRFseries');
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE jobrequestfinalpending
         ...
         mysql_query("update asset_depreciation set price='$price',
          salvage_val='$sv',years='$years',depmed='$method' where item_id='$id'") 
                ...
                <script>
				alert('Job Request Pending Reason Successfully Updated ...');
...
?>
    ...
    <div class="col-sm-10">
   	  <textarea class="form-control" type="text" rows="8" name="disapprovereasons" value="<?php echo $disapprovereasons; ?>" required/><?php echo $disapprovereasons;?></textarea>
    </div>
  </div>  
    ...
    <div class="col-sm-10">
      <input type="text"  name="pendingby"  value="<?php echo $_SESSION[sess_fname];?> <?php echo $_SESSION['sess_lastname'];?>" class="form-control" id="inputPassword3" readonly/>
    </div>
  </div>
