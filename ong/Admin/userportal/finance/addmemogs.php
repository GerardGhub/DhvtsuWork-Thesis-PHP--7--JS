...
$revisionno = $_POST['revisionno'];
 ...
 $check_item="select item_id from memorandum WHERE item_image='item_image'";
...
$itempic = rand(1000,1000000).".".$imgExt;
			...
			if(in_array($imgExt, $valid_extensions)){			
				...
				if($imgSize < 5000000)				{
					...
					$saveitem="insert into memorandum (policy_title,department,item_image,dh,revisionno,item_date) VALUE ('$policy_title','$department','$itempic','$dh','$revisionno',CURDATE())";
					mysqli_query($dbcon,$saveitem);
					 echo "<script>alert('Data successfully saved!')</script>";				
					 echo "<script>window.open('managememogs.php','_self')</script>";
				}
				else{
