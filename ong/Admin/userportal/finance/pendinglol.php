...
//	include('formatMoney.php');
	...
	{
		$id = $_GET['JRFseries'];
		$stmt_edit = $DB_con->prepare('SELECT * FROM jobrequestfinal WHERE JRFseries=:JRFseries');
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE jobrequestfinal
			...
			$stmt->bindParam(':pendingby',$pendingby);
			$stmt->bindParam(':Activity',$Activity);
			$stmt->bindParam(':Activitydate',$Activitydate);
...
$get_id = $_GET['JRFseries'];


$savelisa = "INSERT INTO Jobrequestfinalpending    (Jrfseries,Date_now,Type,Name,Department,JobReq,Purpose,Preparedby,DH,JRFno,MISOIC,Status,pendingby,disapprovereasons)
SELECT Jrfseries,Date_now,Type,Name,Department,JobReq,Purpose,Preparedby,DH,JRFno,MISOIC,Status,pendingby,disapprovereasons FROM Jobrequestfinal WHERE JRFseries='$get_id';";
...
$savelisa = "INSERT INTO Jobrequestfinalactivity    (Jrfseries,Date_now,Type,Name,Department,JobReq,Purpose,Preparedby,DH,JRFno,MISOIC,Status,pendingby,disapprovereasons,Activity,Activitydate)
SELECT Jrfseries,Date_now,Type,Name,Department,JobReq,Purpose,Preparedby,DH,JRFno,MISOIC,Status,pendingby,disapprovereasons,Activity,Activitydate FROM Jobrequestfinal WHERE JRFseries='$get_id';";
...
mysqli_query($connection,"delete from Jobrequestfinal where JRFseries='$get_id'")or die(mysqli_error());
?>
                ...
                <script>
				alert('Job Request Pending Successfully...');
				window.location.href='JRFRequest3.php';
				</script>
                <?php
			}
			else{
...
<!--?php include('../includes/misleftbaruseruser.php');?-->  
                    ...
                    <div class="main-page">
                         ...
                         <h1 class="btn btn-primary" style="font-size:24px; width:1600px;"><emp style="color:whitesmoke;">Reason for Pending a Job Request</emp> <b>
							</b></h1>   	        
...
include('conface.php');
$id=$_GET['JRFseries'];
$query=mysql_query("select * from jobrequestfinal where JRFseries='$id'") or die (mysql_error());
...
?>
    ...
    <div class="col-sm-10">
   	  <textarea class="form-control" type="text" rows="8" name="disapprovereasons" value="<?php echo $disapprovereasons; ?>" required></textarea>
    </div>
  </div>  
...
echo date('l jS \of F Y h:i:s A');?>" style="display:none;">
     </p>
  
  
  
  <input type="text" name="Activity" value="<?php echo$_SESSION['sess_fname'];?> <?php echo $_SESSION['sess_lastname'];?> Pending <?php echo $_SESSION['sess_department'];?> Job Request" style="display:none;">
    ...
    <div class="col-sm-10">
      <input type="text"  name="life"  value="<?php echo $Preparedby;?>" class="form-control" id="inputPassword3" readonly/>
  		  ...
  		  <div class="form-group">
    <label for="inputPassword3" class="col-sm-2 control-label" style="color:black;">Pending BY</label>
    <div class="col-sm-10">
   	  <input class="form-control" type="text" name="pendingby" value="<?php echo $_SESSION['sess_fname']; ?> <?php echo $_SESSION['sess_lastname']; ?>" readonly />
    </div>
  </div>
