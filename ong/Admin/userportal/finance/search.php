...
<div class="container" style="background-color:whitesmoke;">
  <div class="row">
	...
	$query = $conn->query("select  * from ongusers where fname LIKE '%$search%' or lastname  LIKE '%$search%'");
	$count = $query->rowcount();
	if ($count > 0){ 
	while($row = $query->fetch()){
	$posted_by = $row['fname']." ".$row['lastname'];
	$posted_image = $row['user_image'];
	$friend_id = $row['id'];
		$myidko=$_SESSION['sess_user_id'];
	?>
            <div class="col-md-2 col-sm-3 text-center" >
             <img  src="../item_images/userspics/<?php echo $posted_image; ?>" style="width:178px;height:160px" class="img-circle"></a>
            </div>
		...
		<?php $query1 = $conn->query("select * from friends where my_friend_id = '300'");
			  $count1 = $query1->rowcount();
			 // if ($count1 > 0){ echo 'All Ready in Contact List'; }else{
		?>	
								<!--button  class="btn btn-info"><i class="icon-plus-sign"></i> Add to Contact List</button-->
			<!--?php } ?-->
								</div>
                  </h4></div>
				</form>
