...
//	include('formatMoney.php');
	...
	{
		$id = $_GET['Subject_id'];
		$stmt_edit = $DB_con->prepare('SELECT * from subject WHERE Subject_id=:Subject_id');
		$stmt_edit->execute(array(':Subject_id'=>$id));
		$edit_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
		extract($edit_row);
	}
	else
	{
		header("Location: viewmemosubjects.php");
	}
	...
	if(isset($_POST['btn_save_updates']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('UPDATE subject
         ...
         mysql_query("update asset_depreciation set price='$price',
          salvage_val='$sv',years='$years',depmed='$method' where item_id='$id'") 
                ...
                <script>
				alert('Holiday Details Successfully Updated ...');
				window.location.href='viewmemosubjects.php';
				</script>
                <?php
			}
			else{
...
<!--?php include('../includes/misleftbaruseruser.php');?-->  
         ...
         <center><h4> <span class="fa fa-users"></span> <b>Update O&G Subject Memorandum</b></h4></center>
...
include('conface.php');
$id=$_GET['Memo_id'];
$query=mysql_query("select * from holidaymemo where Memo_id='$id'") or die (mysql_error());
...
?>
    ...
    <label for="inputPassword3" class="col-sm-2 control-label" style="color:black;">Day</label>
    <div class="col-sm-10">
   	  <input class="form-control" type="text" name="Day" value="<?php echo $Day; ?>" required />
    </div>
  </div>  
    ...
    <div class="col-sm-10">
   	  <input class="form-control" type="date" name="Date" value="<?php echo $Date ?>" required />
    				   ...
    				   <div class="form-group">
    <label for="inputPassword3" class="col-sm-2 control-label" style="color:black;">Holiday</label>
    <div class="col-sm-10">
   	  <input class="form-control" type="text" name="Holiday" value="<?php echo $Holiday ?>" required />
    </div>
  </div-->  
  
  
   				   <div class="form-group">
    <label for="inputPassword3" class="col-sm-2 control-label" style="color:black;">Memorandum Subject Category</label>
    <div class="col-sm-10">
   	  <input class="form-control" type="text" name="Subject_name" value="<?php echo $Subject_name ?>" required />
    </div>
  </div> 
    ...
    <div class="col-sm-10">
   	  <input class="form-control" type="text" name="Date_added" value="<?php echo $Date_added; ?>" readonly />
    </div>
  </div>
   ...
   <div class="form-group">
    ...
    <div class="col-sm-10">
   	  <input class="form-control" type="text" name="Date_modified" value="<?php echo $Date_modified; ?>" readonly />
    </div>
  </div>  
